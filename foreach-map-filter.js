...
*/
const values = [1, 2, 3];
const doubledValues = values.map(function(number) {
 return number * 2
})
...
*/
const array1 = [1,2,3];
const evenArray = array1.filter(function(number){
    return number % 2 == 0;
})
...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(val) {
        array2.push(val[0] + val[val.length-1]);
    });
    return array2;}
...
function vowelCount(str){
    const lowerArray = str.toLowerCase().split("");
    const vowels = "aeiou"; 
    const obj = {};

    lowerArray.forEach(function(letter){
...
console.log([{first: 'Elie', last:"Schoppik"}, {first: 'Tim', last:"Garcia", isCatOwner: true}, {first: 'Matt', last:"Lane"}, {first: 'Colt', last:"Steele", isCatOwner: true}], 'isCatOwner',true)
/*
...
function removeVowels(str) {
    ...
    return lowerArray.filter(function(letter){
        if(vowels.indexOf(letter) == -1){
            return letter
        }
    });
